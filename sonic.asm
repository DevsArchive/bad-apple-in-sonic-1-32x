...
; ===========================================================================
	...
	include	"Macros.asm"
	include	"_32X/68000 Definitions.i"
...
; ---------------------------------------------------------------------------
; Vector table in MD mode
; ---------------------------------------------------------------------------

vblankJmp	EQU	$FFFFFFB0
vblankAddr	EQU	vblankJmp+2
...
; ---------------------------------------------------------------------------
		...
		dc.l $3F0			; Start of program
		dc.l ErrorTrap-MARSBANK0	; Bus error
		dc.l ErrorTrap-MARSBANK0	; Address error (4)
		dc.l ErrorTrap-MARSBANK0	; Illegal instruction
		dc.l ErrorTrap-MARSBANK0	; Division by zero
		dc.l ErrorTrap-MARSBANK0	; CHK exception
		dc.l ErrorTrap-MARSBANK0	; TRAPV exception (8)
		dc.l ErrorTrap-MARSBANK0	; Privilege violation
		dc.l ErrorTrap-MARSBANK0	; TRACE exception
		dc.l ErrorTrap-MARSBANK0	; Line-A emulator
		dc.l ErrorTrap-MARSBANK0	; Line-F emulator (12)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved) (16)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved) (20)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved) (24)
		dc.l ErrorTrap-MARSBANK0	; Spurious exception
		dc.l ErrorTrap-MARSBANK0	; IRQ level 1
		dc.l ErrorTrap-MARSBANK0	; IRQ level 2
		dc.l ErrorTrap-MARSBANK0	; IRQ level 3 (28)
		dc.l HBlank-MARSBANK0		; IRQ level 4 (horizontal retrace interrupt)
		dc.l ErrorTrap-MARSBANK0	; IRQ level 5
		dc.l vblankJmp			; IRQ level 6 (vertical retrace interrupt)
		dc.l ErrorTrap-MARSBANK0	; IRQ level 7 (32)
		dc.l ErrorTrap-MARSBANK0	; TRAP #00 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #01 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #02 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #03 exception (36)
		dc.l ErrorTrap-MARSBANK0	; TRAP #04 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #05 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #06 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #07 exception (40)
		dc.l ErrorTrap-MARSBANK0	; TRAP #08 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #09 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #10 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #11 exception (44)
		dc.l ErrorTrap-MARSBANK0	; TRAP #12 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #13 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #14 exception
		dc.l ErrorTrap-MARSBANK0	; TRAP #15 exception (48)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
		dc.l ErrorTrap-MARSBANK0	; Unused (reserved)
...
Checksum: 	dc.w $0
		dc.b "J               " ; I/O support
RomStartLoc:	dc.l $000000		; Start address of ROM
RomEndLoc:	dc.l $3FFFFF		; End address of ROM
...
; ---------------------------------------------------------------------------
; Vector table in 32X mode
; ---------------------------------------------------------------------------

		jmp	(EntryPoint).l		; Start of program
		jmp	(ErrorTrap).l		; Bus error
		jmp	(ErrorTrap).l		; Address error (4)
		jmp	(ErrorTrap).l		; Illegal instruction
		jmp	(ErrorTrap).l		; Division by zero
		jmp	(ErrorTrap).l		; CHK exception
		jmp	(ErrorTrap).l		; TRAPV exception (8)
		jmp	(ErrorTrap).l		; Privilege violation
		jmp	(ErrorTrap).l		; TRACE exception
		jmp	(ErrorTrap).l		; Line-A emulator
		jmp	(ErrorTrap).l		; Line-F emulator (12)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved) (16)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved) (20)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved) (24)
		jmp	(ErrorTrap).l		; Spurious exception
		jmp	(ErrorTrap).l		; IRQ level 1
		jmp	(ErrorTrap).l		; IRQ level 2
		jmp	(ErrorTrap).l		; IRQ level 3 (28)
		jmp	(HBlank).l		; IRQ level 4 (horizontal retrace interrupt)
		jmp	(ErrorTrap).l		; IRQ level 5
		jmp	(vblankJmp).w		; IRQ level 6 (vertical retrace interrupt)
		jmp	(ErrorTrap).l		; IRQ level 7 (32)
		jmp	(ErrorTrap).l		; TRAP #00 exception
		jmp	(ErrorTrap).l		; TRAP #01 exception
		jmp	(ErrorTrap).l		; TRAP #02 exception
		jmp	(ErrorTrap).l		; TRAP #03 exception (36)
		jmp	(ErrorTrap).l		; TRAP #04 exception
		jmp	(ErrorTrap).l		; TRAP #05 exception
		jmp	(ErrorTrap).l		; TRAP #06 exception
		jmp	(ErrorTrap).l		; TRAP #07 exception (40)
		jmp	(ErrorTrap).l		; TRAP #08 exception
		jmp	(ErrorTrap).l		; TRAP #09 exception
		jmp	(ErrorTrap).l		; TRAP #10 exception
		jmp	(ErrorTrap).l		; TRAP #11 exception (44)
		jmp	(ErrorTrap).l		; TRAP #12 exception
		jmp	(ErrorTrap).l		; TRAP #13 exception
		jmp	(ErrorTrap).l		; TRAP #14 exception
		jmp	(ErrorTrap).l		; TRAP #15 exception (48)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved)
		jmp	(ErrorTrap).l		; Unused (reserved)
...
; ---------------------------------------------------------------------------
		...
		dc.l	SH2Start-MARSBANK0	; SH-2 start address (within full 4 MB ROM space)
		dc.l	0			; SH-2 program write address (MCD only)
		dc.l	SH2End-SH2Start		; SH-2 program length
		
		dc.l	$6000120		; Master SH-2 entry point
		dc.l	$6000520		; Slave SH-2 entry point
		dc.l	$6000000		; Master SH-2 VBR
		dc.l	$6000400		; Slave SH-2 VBR

; ---------------------------------------------------------------------------
; MARS initialization/security
; ---------------------------------------------------------------------------

		incbin	"_32X/Security.bin"
		bcs.s	@Failed
		
		lea	MARSSYSREG,a0
		
@WaitMaster:
		cmpi.l	#"M_OK",MARSCOMM0(a0)		; Wait for the master SH-2 to be ready
		bne.s	@WaitMaster
		
@WaitSlave:
		cmpi.l	#"S_OK",MARSCOMM4(a0)		; Wait for the slave SH-2 to be ready
		bne.s	@WaitSlave
		
		moveq	#0,d0				; Tell the SH-2's we're ready
		move.l	d0,MARSCOMM0(a0)
		move.l	d0,MARSCOMM4(a0)
		
		bra.w	GameProgram
		
@Failed:
		move.l	#$C0000000,vdp_control_port	; Halt at a red screen
...
GameInit:
	...
	@clearRAM:
		...
		move.w	#$4EF9,vblankJmp.w
		move.l	#VBlankOff,vblankAddr.w
...
VBlank:
		...
		move.l	(v_scrposy_dup).w,vdp_data_port ; send screen y-axis pos. to VSRAM
...
JoypadInit:
		...
		move.b	d0,$A1000D	; init port 3 (expansion/extra)
		startZ80
		rts	
...
SoundDriverLoad:
		...
		stopZ80
		resetZ80
		lea	Kos_Z80,a0	; load sound driver
		lea	z80_ram,a1	; target Z80 RAM
...
QuickPLC:
		lea	ArtLoadCues,a1 ; load the PLC index
		add.w	d0,d0
		move.w	(a1,d0.w),d0
		lea	(a1,d0.w),a1
	...
	Qplc_Loop:
		...
		ori.w	#$4000,d0
		swap	d0
		move.l	d0,vdp_control_port ; converted VRAM address to VDP format
...
GM_Sega:
		...
		bsr.w	ClearScreen
		locVRAM	0
		lea	Nem_SegaLogo,a0 ; load Sega logo patterns
		bsr.w	NemDec
		lea	$FF0000,a1
		lea	Eni_SegaLogo,a0 ; load Sega logo mappings
		move.w	#0,d0
		bsr.w	EniDec
...
GM_Title:
	...
	Tit_ClrObj1:
		...
		locVRAM	0
		lea	Nem_JapNames,a0 ; load Japanese credits
		bsr.w	NemDec
		locVRAM	$14C0
		lea	Nem_CreditText,a0 ;	load alphabet
		bsr.w	NemDec
		lea	$FF0000,a1
		lea	Eni_JapNames,a0 ; load mappings for	Japanese credits
		move.w	#0,d0
		bsr.w	EniDec
	...
	Tit_ClrPal:
		...
		locVRAM	$4000
		lea	Nem_TitleFg,a0 ; load title screen patterns
		bsr.w	NemDec
		locVRAM	$6000
		lea	Nem_TitleSonic,a0 ; load Sonic title screen	patterns
		bsr.w	NemDec
		locVRAM	$A200
		lea	Nem_TitleTM,a0 ; load "TM" patterns
		bsr.w	NemDec
		lea	vdp_data_port,a6
		locVRAM	$D000,4(a6)
		lea	Art_Text,a5	; load level select font
		move.w	#$28F,d1
	...
	Tit_LoadText:
		...
		bsr.w	DeformLayers
		lea	(v_16x16).w,a1
		lea	Blk16_GHZ,a0 ; load	GHZ 16x16 mappings
		move.w	#0,d0
		bsr.w	EniDec
		lea	Blk128_GHZ,a0 ; load GHZ 128x128 mappings
		lea	v_128x128,a1
		bsr.w	KosDec
		...
		lea	(v_layout+$80).w,a4	; MJ: Load address of layout BG
		move.w	#$6000,d2
		bsr.w	DrawChunks
		lea	$FF0000,a1
		lea	Eni_Title,a0 ; load	title screen mappings
		move.w	#0,d0
		bsr.w	EniDec
		...
		locVRAM	0
		lea	Nem_GHZ_1st,a0 ; load GHZ patterns
		bsr.w	NemDec
...
Tit_ChkRegion:
		...
		lea	LevSelCode_US,a0 ; load US code
		bra.s	Tit_EnterCheat

	Tit_RegionJap:
		lea	LevSelCode_J,a0 ; load J code
...
Level_LoadObj:
	...
	Level_SkipClr:
		...
		lea	DemoDataPtr,a1 ; load demo data
		moveq	#0,d0
		move.b	(v_zone).w,d0
		lsl.w	#2,d0
		movea.l	(a1,d0.w),a1
		...
		lea	DemoEndDataPtr,a1 ; load ending demo data
...
loc_47D4:
		...
		jsr	PlaySound_Special	 ; play end-of-level music
...
SS_BGLoad:
		lea	$FF0000,a1
		lea	Eni_SSBg1,a0 ; load	mappings for the birds and fish
		move.w	#$4051,d0
		bsr.w	EniDec
		move.l	#$50000001,d3
		lea	$FF0080,a2
...
loc_491C:
		...
		dbf	d7,loc_48BE
		lea	$FF0000,a1
		lea	Eni_SSBg2,a0 ; load	mappings for the clouds
		move.w	#$4000,d0
		bsr.w	EniDec
		lea	$FF0000,a1
		move.l	#$40000003,d0
...
loc_4992:
		...
		move.w	d0,(a6)
		move.l	#$40000010,vdp_control_port
		move.l	(v_scrposy_dup).w,vdp_data_port
		moveq	#0,d0
		move.b	(a0)+,d0
		bmi.s	loc_49E8
		lea	Pal_SSCyc1,a1
...
GM_Continue:
	...
	Cont_ClrObjRam:
		...
		bsr.w	NemDec
		locVRAM	$A000
		lea	Nem_ContSonic,a0 ; load Sonic patterns
		bsr.w	NemDec
		locVRAM	$AA20
		lea	Nem_MiniSonic,a0 ; load continue screen patterns
		bsr.w	NemDec
		moveq	#10,d1
...
End_LoadData:
		...
		lea	Kos_EndFlowers,a0 ;	load extra flower patterns
...
End_ChkEmerald:
	...
	End_SlowFade:
		...
		move.l	#$AAABAE9A,(v_layout+$200).w ; MJ: set extra flowers version of ending's layout to be read
		move.l	#$ACADAFB0,(v_layout+$300).w
		lea	vdp_control_port,a5
		lea	vdp_data_port,a6
		lea	(v_screenposx).w,a3
		lea	(v_layout).w,a4	; MJ: Load address of layout
		move.w	#$4000,d2
		bsr.w	DrawChunks
...
GM_Credits:
	...
	Cred_ClrObjRam:
		...
		locVRAM	$B400
		lea	Nem_CreditText,a0 ;	load credits alphabet patterns
		bsr.w	NemDec
...
EndingDemoLoad:
		...
		lea	EndDemo_LampVar,a1 ; load lamppost variables
		lea	(v_lastlamp).w,a2
		move.w	#8,d0
...
LevelDataLoad:
		...
		bsr.w	EniDec
		movea.l	(a2)+,a0
		lea	v_128x128,a1 ; RAM address for 128x128 mappings
		bsr.w	KosDec
		bsr.w	LevelLayoutLoad
		move.w	(a2)+,d0
		move.w	(a2),d0
...
LevelLayoutLoad:
		...
		lsr.w	#4,d0
		lea	Level_Index,a1
		movea.l	(a1,d0.w),a0		; MJ: moving the address strait to a1 rather than adding a word to an address
		lea	(v_layout).w,a1
		jmp	KosDec
...
loc_84F2:
		...
		jmp	PlaySound_Special	; play collapsing sound
; ===========================================================================
; ---------------------------------------------------------------------------
...
; ---------------------------------------------------------------------------
		...
		lea	RawColBlocks,a1		; Source location of raw collision block data
		lea	ConvRowColBlocks,a2	; Destinatation location for row-converted collision block data
	...
	@rowLoop:
		...
		lea	ConvRowColBlocks,a1
		lea	CollArray2,a2		; Convert the row-converted collision block data into final rotated collision array
		bsr.s	@convertArray
		lea	RawColBlocks,a1
		lea	CollArray1,a2		; Convert the raw collision block data into final normal collision array
...
SS_AniEmeraldSparks:
		...
		jsr	PlaySound_Special	; play special stage GOAL sound
...
Blk128_MZ:	if Revision=0
		incbin	"map128\MZ.bin"
		else
		incbin	"map128\MZ (JP1).bin"
		endc
		even
...
Blk128_SBZ:	if Revision=0
		incbin	"map128\SBZ.bin"
		else
		incbin	"map128\SBZ (JP1).bin"
		endc
		even
; ---------------------------------------------------------------------------
...
Level_GHZ1:	incbin	"levels\ghz1.kos"
		even
Level_GHZ2:	incbin	"levels\ghz2.kos"
		even
Level_GHZ3:	incbin	"levels\ghz3.kos"
		even

Level_LZ1:	incbin	"levels\lz1.kos"
		even
Level_LZ2:	incbin	"levels\lz2.kos"
		even
Level_LZ3:	incbin	"levels\lz3.kos"
		even
Level_SBZ3:	incbin	"levels\sbz3.kos"
		even

Level_MZ1:	incbin	"levels\mz1.kos"
		even
Level_MZ2:	incbin	"levels\mz2.kos"
		even
Level_MZ3:	incbin	"levels\mz3.kos"
		even

Level_SLZ1:	incbin	"levels\slz1.kos"
		even
Level_SLZ2:	incbin	"levels\slz2.kos"
		even
Level_SLZ3:	incbin	"levels\slz3.kos"
		even

Level_SYZ1:	incbin	"levels\syz1.kos"
		even
Level_SYZ2:	incbin	"levels\syz2.kos"
		even
Level_SYZ3:	incbin	"levels\syz3.kos"
		even

Level_SBZ1:	incbin	"levels\sbz1.kos"
		even
Level_SBZ2:	incbin	"levels\sbz2.kos"
		even

Level_End:	incbin	"levels\ending.kos"
		even
...
; ---------------------------------------------------------------------------
		...
		if PAL=0
			incbin	"_BadApple/Anim60.bin"
		else
			incbin	"_BadApple/Anim50.bin"
		endif
		align	$80000

; ---------------------------------------------------------------------------

BadApplePWM	EQU	(*-MARSBANK0)/$80000
		incbin	"_BadApple/Audio.bin"
		align	$80000
BadApplePWMEnd	EQU	((*-MARSBANK0)/$80000)
